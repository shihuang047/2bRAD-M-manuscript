...
setwd("./")
source("util.R")
prefix="FFPE_quan_G5_69f"
pred.prefix="FFPE_quan_G5_69f"
...
#-------------------------------
dir.create(outpath<-paste("./",file.opts$prefix,"-",file.opts$pred.prefix,".",file.opts$group.type,"2/",sep=""))
#-------------------------------
# data format
#-------------------------------
# matrix: 
...
con <- file(paste(outpath,file.opts$prefix,"-",file.opts$pred.prefix,".",file.opts$pred.group.type,".",rf.opts$n.oob,".log",sep=""))
sink(con, append=TRUE)
sink(con, append=TRUE, type="message")
#-------------------------------
# Training data input
#-------------------------------
g<-read.table(paste(filename,".txt",sep=""),header=T,row.names=1,sep="\t")
...
# Test data input
#-------------------------------
pred.g<-read.table(paste(pred.filename,".txt",sep=""),header=T,row.names=1,sep="\t")
...
kappa.result.cv<-cohen.kappa(result$confusion.matrix)
...
#---------------------------   
sink(paste(outpath,file.opts$prefix,".",file.opts$group.type,".Prob.xls",sep="")); cat("\t"); write.table(result$probabilities,sep="\t",quote=FALSE); sink()
#----------------------------probability_boxplot
pdf(paste(outpath,file.opts$prefix,".",file.opts$group.type,".probability_boxplot.pdf",sep=""), height=8, width=4)
par(mar=c(4,4,3,3))
...
if(nlevels(group)==2){
  ...
  #--------------------------------------------------
  pdf(paste(outpath,file.opts$prefix,".",file.opts$group.type,".Prob.pROC.ci.pdf",sep=""),width=5,height=5)
...
#--------------------------------------------------
result <- rf.out.of.bag(x, y, verbose=rf.opts$verbose, ntree=rf.opts$ntree)
print(paste("The out-of-bag (3-fold CV) result for randomforest classification",sep=""))
#----------------------------
print(result$confusion.matrix)
kappa.result.outofbag<-cohen.kappa(result$confusion.matrix)
cat(paste("Cohen's kappa statistics for agreement in out-of-bag classification: ", kappa.result.outofbag$Result$estimate," -- ",kappa.result.outofbag$Judgement,"\n",sep=""))
#----------------------------
predicted.prob<-predict(result$rf.model,pred.gmatO,type="prob")      
sink(paste(outpath,file.opts$pred.prefix,".",file.opts$pred.group.type,".Pred_Prob.xls",sep="")); cat("\t"); write.table(predicted.prob,sep="\t",quote=FALSE); sink()
#----------------------------probability_boxplot
pdf(paste(outpath,file.opts$pred.prefix,".",file.opts$pred.group.type,".probability_boxplot.pdf",sep=""), height=8, width=4)
par(mar=c(4,4,3,3))
...
if(nlevels(group)==2){
  #----------------------------probability_xyplot
  pdf(paste(outpath,file.opts$pred.prefix,".",file.opts$pred.group.type,".probability_xyplot.pdf",sep=""), height=6, width=6)
  par(mar=c(4,4,3,3))
  ...
  if(nlevels(pred.group)>1){
    ...
    auc.tmp <- performance(pred,"auc"); auc <- as.numeric(auc.tmp@y.values)
    #----------------------------
    pdf(paste(outpath,file.opts$pred.prefix,".",file.opts$pred.group.type,".ROCR_plot.pdf",sep=""), height=4, width=4)
    par(mar=c(4,4,3,3))
    ...
    dev.off()
    #----------------------------
    pdf(paste(outpath,file.opts$prefix,".",file.opts$pred.group.type,".ROCR.accuracy_VS_cutoff.plot.pdf",sep=""), height=4, width=4)
    par(mar=c(4,4,3,3))
    acc.perf <- performance(pred, "acc")
    plot(acc.perf, avg= "vertical")
    dev.off()
...
if(file.opts$pred.group.type==file.opts$pred.group.type){
  ...
  comb.prob<-rbind(result$probabilities,predicted.prob)
...
if(file.opts$rf.cv){
  ...
  #--------------------------------------------------
  results <- replicate(10, rfcv(gmatO, group, step=0.9,cv.fold=10), simplify=FALSE)
  ...
  ggsave(filename=paste(outpath,file.opts$prefix,".CV.MeanErrPlot.ggplot.pdf",sep=""),plot=p,width=3,height=4)
  #-------------------------------------------------MedianErr
...
if(nlevels(group)==2){
  ...
  sink(paste(outpath,file.opts$prefix,".",file.opts$group.type,".imps_wilcox.",nrow(test.results),".xls",sep=""))
  ...
  AUCs<-c()
  for(i in 1:ncol(x)){auc1<-auc(y,x[,i]);AUCs<-c(AUCs,auc1)}
  ...
  #-------------------------------
  AUCs<-as.matrix(AUCs)
  rownames(AUCs)<-colnames(x)
  AUCs=AUCs[order(rownames(AUCs)),]
  ...
  #ggsave(filename=paste(outpath,file.opts$prefix,".AUC_VS_ImpScore.",ncol(x),".ColByWilcox",nrow(TaxaSumm.minErr),".pdf",sep=""),plot=p,width=2.5,height=4)
  #-------------------------------
  ...
  if(nrow(wilcoxEnr)>0){
    ...
    log.mat_Group<-data.frame(Group=group1[rownames(TaxaSumm.minErr),],log.gmat[rownames(TaxaSumm.minErr),order(colnames(log.gmat))])
    ...
    ggsave(filename=paste(outpath,file.opts$prefix,".impVars.wilcox.",nrow(wilcoxEnr),".boxplot.pdf",sep=""),plot=p,limitsize = FALSE,width=3,height=4)
